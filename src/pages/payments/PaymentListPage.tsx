...
interface CashFlowStats {
  ...
  byPaymentMethod: Record<string, { income: number; expense: number; total: number }>
  byPaymentType: Record<string, { income: number; expense: number; total: number }>
...
export default function PaymentListPage() {
  ...
  const calculateStats = (paymentsData: Payment[]) => {
    const stats = paymentsData.reduce((acc, payment) => {
      ...
      // Por método de pago
      if (!acc.byPaymentMethod[payment.paymentMethod]) {
        acc.byPaymentMethod[payment.paymentMethod] = { income: 0, expense: 0, total: 0 }
      }
      
      if (payment.type === 'I') {
        acc.byPaymentMethod[payment.paymentMethod].income += amount
      } else {
        acc.byPaymentMethod[payment.paymentMethod].expense += amount
      }
      acc.byPaymentMethod[payment.paymentMethod].total = 
        acc.byPaymentMethod[payment.paymentMethod].income - 
        acc.byPaymentMethod[payment.paymentMethod].expense
      
      // Por tipo de pago (contado/crédito)
      if (!acc.byPaymentType[payment.paymentType]) {
        acc.byPaymentType[payment.paymentType] = { income: 0, expense: 0, total: 0 }
      }
      
      if (payment.type === 'I') {
        acc.byPaymentType[payment.paymentType].income += amount
      } else {
        acc.byPaymentType[payment.paymentType].expense += amount
      }
      acc.byPaymentType[payment.paymentType].total = 
        acc.byPaymentType[payment.paymentType].income - 
        acc.byPaymentType[payment.paymentType].expense
      ...
      pendingPayments: 0,
      completedPayments: 0,
      byPaymentMethod: {} as Record<string, { income: number; expense: number; total: number }>,
      byPaymentType: {} as Record<string, { income: number; expense: number; total: number }>,
      netBalance: 0,
      filteredTotal: 0,
      filteredIncome: 0,
      filteredExpense: 0
  ...
  const filteredStats = filteredPayments.reduce((acc, payment) => {
    const amount = Number(payment.paidAmount) || 0;
    if (payment.type === 'I') {
      acc.filteredIncome += amount
    } else {
      acc.filteredExpense += amount
    }
    acc.filteredTotal = acc.filteredIncome - acc.filteredExpense
    return acc
  }, { filteredIncome: 0, filteredExpense: 0, filteredTotal: 0 })
   ...
   <div className="h-full bg-slate-50 flex flex-col font-['Inter',_'system-ui',_sans-serif]">
     ...
     <div className="px-4 py-2.5">
       <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-lg p-3 shadow-sm">
         ...
         {/* Totales por método de pago si hay filtro activo */}
         {methodFilter !== 'all' && stats.byPaymentMethod[methodFilter] && (
           <div className="mt-3 pt-3 border-t border-white/10">
             <div className="flex items-center justify-between">
               <div className="flex items-center space-x-2">
                 <div className="p-1 bg-white/10 rounded">
                   {PAYMENT_METHOD_ICONS[methodFilter]}
                 </div>
                 <div>
                   <p className="text-xs font-medium text-slate-300">
                     Total {PAYMENT_METHOD_LABELS[methodFilter]}
                   </p>
                   <div className="flex items-center space-x-3 mt-0.5">
                     <span className="text-xs text-emerald-400">
                       +{formatCurrency(stats.byPaymentMethod[methodFilter].income)}
                     </span>
                     <span className="text-xs text-red-400">
                       -{formatCurrency(stats.byPaymentMethod[methodFilter].expense)}
                     </span>
                     <span className={`text-sm font-bold ${
                       stats.byPaymentMethod[methodFilter].total >= 0 ? 'text-cyan-400' : 'text-orange-400'
                     }`}>
                       = {formatCurrency(stats.byPaymentMethod[methodFilter].total)}
                     </span>
                   </div>
                 </div>
               </div>
         ...
         {/* Mostrar totales filtrados si hay filtros activos */}
         {(typeFilter !== 'all' || statusFilter !== 'all' || searchTerm) && (
           <div className="mt-3 pt-3 border-t border-white/10">
             <div className="flex items-center justify-between">
               <div className="flex items-center space-x-2">
                 <Calculator className="w-4 h-4 text-purple-400" />
                 <p className="text-xs font-medium text-slate-300">Totales filtrados:</p>
               </div>
               <div className="flex items-center space-x-3">
                 <span className="text-xs text-emerald-400">
                   +{formatCurrency(filteredStats.filteredIncome)}
                 </span>
                 <span className="text-xs text-red-400">
                   -{formatCurrency(filteredStats.filteredExpense)}
                 </span>
                 <span className={`text-sm font-bold ${
                   filteredStats.filteredTotal >= 0 ? 'text-cyan-400' : 'text-orange-400'
                 }`}>
                   = {formatCurrency(filteredStats.filteredTotal)}
                 </span>
               </div>
             </div>
           </div>
